...
## Fran's attempt to increase performance


randomize.peaks <- function(input.peaks,chr.lengths)
{
  random.peaks <- matrix(nrow=nrow(input.peaks),ncol=3) #Matriz con 3 columnas, una para el cromosoma, otra para el comienzo y otra para el final de la región aleatoria.
  for(k in 1:nrow(input.peaks))
  {
    current.chr <- input.peaks[k,1][[1]] #Chr de la iésima marca real
    current.start <- input.peaks[k,2] #Start de la iésima marca real
    current.end <- input.peaks[k,3] #End de la iésima marca real
    current.length <- current.end - current.start #Longitud de la iésima marca real
    
    chr.length <- chr.lengths[current.chr] #Length del actual cromosoma
    #Ahora genero los mismos datos para regiones aleatorias
    random.start <- floor(runif(n = 1,min = 1,max = chr.length))
    random.end <- random.start + current.length
    
    random.peaks[k,1] <- current.chr
    random.peaks[k,2] <- random.start
    random.peaks[k,3] <- random.end
  }
  return(random.peaks)
}

randomize.peaks.fran <- function(input.peaks,chr.lengths)
{
...
start.time <- Sys.time()
res <- randomize.peaks(input.peaks = peaks2,chr.lengths = chromosomes.length)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken

start.time <- Sys.time()
res <- randomize.peaks.fran(input.peaks = peaks2,chr.lengths = chromosomes.length)
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken
...
# Time difference of 2.073841 mins
